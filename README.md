...
3. 고객이 결제한다.
4. 시스템이 예약 결과를 고객에게 발송한다.
5. 시스템이 예약 되면 티켓좌석이 감소된다.

1. 고객이 예약 내역을 열람한다.
2. 고객이 예약을 취소한다.
3. 시스템이 결제를 취소한다.
4. 시스템이 예약 취소 결과를 고객에게 발송한다.
5. 시스템이 예약 취소 외면 티켓좌석이 증가한다.
...
2. 고객이 영화를 선택하여 예약한다.
3. 예약과 동시에 결제가 진행된다.
4. 결제 완료후 예약승인확인(confirm)이 되면 예약 승인내역(Kakao Message)이 전달된다.
5. 고객이 예약을 취소할 수 있다.
6. 결제 취소후 예약취소확인(cancel)이 되면 예약 취소내역(Kakao Message)이 전달된다.
7. 영화관람 후 후기(review)를 남길 수 있다.
8. 전체적인 영화 정보 및 예약 상태 등을 한 화면에서 확인 할 수 있다.(dashbord)
...
장애격리
영화예약 등록 및 메시지 전송 기능이 수행되지 않더라도 예약은 할수 있어야 한다 Async (event-driven), Eventual Consistency
예약 시스템이 과중되면 사용자를 잠시동안 받지 않고 잠시 후에 하도록 유도한다 Circuit breaker, fallback
성능
모든 영화 예약정보 및 예약 상태 등을 한번에 확인할 수 있어야 한다 (CQRS)
